...
static void I2CRead(uint8_t addr, uint8_t* buf, uint8_t len) {
    i2c_read_bytes_no_stop(pcf8563_device, addr, buf, len);
}
...
esp_err_t PCF8563_Init(i2c_port_t i2c_num, gpio_num_t sda, gpio_num_t scl, uint32_t baud) {
    pcf8563_device = i2c_malloc_device(i2c_num, sda, scl, baud, PCF8563_ADDR);
    if (pcf8563_device == NULL) {
