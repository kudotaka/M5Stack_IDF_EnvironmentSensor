...
static I2CDevice_t qmp6988_device = NULL;
...
void Qmp6988_SetOversamplingT(unsigned char oversampling_t) {
    ...
    i2c_read_bytes(qmp6988_device, (uint32_t)QMP6988_CTRLMEAS_REG, &(data), (uint16_t)1);
    data &= 0x1f;
    data |= (oversampling_t << 5);
    i2c_write_bytes(qmp6988_device, (uint32_t)QMP6988_CTRLMEAS_REG, &(data), (uint16_t)1);
...
void Qmp6988_SetFilter(unsigned char filter) {
    ...
    data = (filter & 0x03);
    i2c_write_bytes(qmp6988_device, (uint32_t)QMP6988_CONFIG_REG, &(data), (uint16_t)1);
...
void Qmp6988_SoftwareReset() {
    ...
    esp_err_t ret = ESP_OK;
    data = 0xe6;
    ret = i2c_write_bytes(qmp6988_device, (uint32_t)QMP6988_RESET_REG, &(data), (uint16_t)1);
    if (ret != ESP_OK) {
...
esp_err_t Qmp6988_GetChipID() {
    ...
    ret = i2c_read_bytes(qmp6988_device, (uint32_t)QMP6988_CHIP_ID_REG, &(qmp6988.chip_id), (uint16_t)1);
    vTaskDelay( pdMS_TO_TICKS(100) );
    if (ret == ESP_OK) {
